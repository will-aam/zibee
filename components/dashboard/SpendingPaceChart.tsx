...
export function SpendingPaceChart({
  ...
  const chartData = useMemo(() => {
    // Garantia de que trabalhamos com arrays, evitando o erro de .filter
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      data.push({
        day: i,
        atual: currentDaySpend,
        passado: accumPassado,
      });
    }
    return data;
  }, [
  ...
  if (!isMounted)
    return (
      <div className="h-[250px] w-full animate-pulse bg-muted/20 rounded-3xl" />
    );

  return (
    <div className="bg-card border border-border/50 rounded-3xl p-5 sm:p-6 shadow-sm overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        ...
        <div
          className={`flex items-center gap-2 px-3 py-1.5 rounded-2xl border self-start ${
            metrics.isBad
                  ...
                  return (
                    <div className="bg-background/95 backdrop-blur-md border border-border p-3 rounded-2xl shadow-xl z-9999">
                      ...
                      <div className="space-y-1">
                        {dataHoje !== undefined && dataHoje !== null && (
                          <p className="text-sm font-bold text-primary">
                            Hoje: {formatMoney(Number(dataHoje))}
                          </p>
                        )}
                        <p className="text-sm font-medium text-muted-foreground/70">
                          Mês Passado: {formatMoney(Number(dataPassado || 0))}
                        </p>
                      </div>
                    </div>
                  );
